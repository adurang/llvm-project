...
Error offloading::intel::containerizeOpenMPSPIRVImage(
    std::unique_ptr<MemoryBuffer> &Img, StringRef CompileOpts,
    StringRef LinkOpts) {
  ...
  InnerImage.TheOffloadKind = OffloadKind::OFK_OpenMP;
  InnerImage.Flags = 0;

  // Migrate metadata from ELF notes to OffloadBinary string table
  // Note: We need to store strings that persist, not temporaries
  MapVector<StringRef, StringRef> StringData;
  StringData["version"] = "1.0";
  StringData["format"] = "spirv";
  StringData["triple"] = "spirv64-intel";
  StringData["arch"] = "unknown";

  // Store compile/link options if provided (migrated from ELF notes)
