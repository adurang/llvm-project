...
Error L0ProgramBuilderTy::buildModules(const std::string_view BuildOptions) {
  ...
  if (identify_magic(Image.getBuffer()) == file_magic::offload_binary) {
    ODBG(OLDT_Module) << "Processing nested OffloadBinary format\n";
    ...
    if (ImageKind == llvm::object::IMG_SPIRV) {
      ...
      ODBG(OLDT_Module) << "Loading SPIR-V IL module\n";
  ...
  if (identify_magic(Image.getBuffer()) == file_magic::spirv_object) {
    ...
    ODBG(OLDT_Module) << "Processing raw SPIR-V (legacy format)\n";
  ...
  // Handle legacy ELF-wrapped SPIR-V format (deprecated)
  ...
  ODBG(OLDT_Module) << "Processing ELF-wrapped SPIR-V (legacy format)\n";
